      ...
      <div className="max-w-5xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <span className="text-xs px-3 py-1 rounded-full font-semibold mb-3 inline-block"
            style={{ backgroundColor: `${color}30`, color }}>Course</span>
        ...
        <input type="text" placeholder={`Search ${categoryName} notes...`} value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="w-full max-w-xl rounded-xl px-4 py-3 text-white placeholder-gray-400 focus:outline-none mb-8"
          style={{ background: "rgba(255,255,255,0.08)", backdropFilter: "blur(10px)", border: `1px solid ${color}40` }} />

        {loading ? (
          <p className="text-gray-300 animate-pulse">Loading notes...</p>
        ) : filtered.length === 0 ? (
          <p className="text-gray-400 text-lg mt-8">{search ? "Koi note nahi mila." : `${categoryName} ke notes abhi upload nahi hue.`}</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((note, i) => (
              <motion.div key={note.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.4, delay: i * 0.07 }} whileHover={{ scale: 1.03 }}
                className="rounded-2xl p-6 flex flex-col justify-between transition-all duration-300"
                style={{ background: "rgba(255,255,255,0.07)", backdropFilter: "blur(20px)", border: `1px solid ${color}30`, borderTop: `3px solid ${color}` }}>
                <div>
                  <h3 className="text-lg font-bold text-white mb-1">{note.title}</h3>
                  {Array.isArray(note.content) && <p className="text-gray-500 text-xs">{note.content.length} sections</p>}
                </div>
                <div className="flex gap-2 mt-5">
                  <a href={`/notes/${note.id}`}
                    className="flex-1 py-2 rounded-xl text-sm font-semibold text-white text-center transition-all hover:opacity-90"
                    style={{ backgroundColor: color }}>View</a>
                  <button
                    className="flex-1 py-2 rounded-xl text-sm font-semibold transition-all"
                    style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)" }}
                    onClick={() => { if (note.pdfUrl) window.open(note.pdfUrl, "_blank"); else alert("PDF nahi hai!"); }}>
